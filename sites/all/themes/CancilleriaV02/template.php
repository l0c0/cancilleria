...
<?php
	...
	function print_comment($vars) {
		...
		$$name = & $vars[$name];
?>
<div id="comments" class="Cancilleria_comments">
	<div class="Cancilleria_comment Cancilleria_postcontent <?php print ($comment->new) ? ' comment-new' : ''; print ' '. $status; print ' '. $zebra; ?>">
		<h3 class="Cancilleria_postheader">
			<?php print $title ?>
		</h3>
		<div class="Cancilleria_comment-avatar">
			<?php if (isset($picture) && $picture != '') print $picture;
			else print '<img src="'.get_full_path_to_theme().'/images/no-avatar.jpg" alt="No Avatar">'; ?>
		</div>

		<div class="Cancilleria_comment-inner">
			<div class="Cancilleria_comment-header">
				<?php print $submitted; ?>
				<?php if ($comment->new) : ?>
					<span class="new"><?php print drupal_ucfirst($new) ?></span>
				<?php endif; ?>
			</div>
			<div class="Cancilleria_comment-content content">
				<?php print $content ?>
				<?php if ($signature): ?>
			...
			<?php if ($links): ?>
				<div class="Cancilleria_comment-footer">
					<?php print str_replace('<a href="', '<a class="Cancilleria_button" href="', $links); ?>
				</div>
			<?php endif; ?>
		</div>
	</div>
...
<?php
	...
	function print_comment($vars) {
		foreach (array_keys($vars) as $name)
			$$name = & $vars[$name];
?>
<div class="Cancilleria_comment Cancilleria_postcontent <?php print $classes; ?>" <?php print $attributes; ?>>
  ...
  <div class="Cancilleria_comment-inner">
    ...
    <div class="Cancilleria_comment-content content" <?php print $content_attributes; ?>>
		<?php
    ...
    <div class="Cancilleria_comment-footer">
      <?php $links = render($content['links']);
				print str_replace('<a href="', '<a class="Cancilleria_button" href="', $links);
			?>
    </div>
  </div>
</div>
...
<div id="comments" class="Cancilleria_comments <?php print $classes; ?>" <?php print $attributes; ?>>
  <?php if ($content['comments'] && $node->type != 'forum'): ?>
  <?php print render($title_prefix); ?>
  <h2 class="Cancilleria_postheader">
...
<div id="comments" class="Cancilleria_commentsform">
  <h3>
