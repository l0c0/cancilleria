...
function art_node_title_output($title, $node_url, $page) {
  $output = '';
  if (!$page)
    $output = "<h2 class='Cancilleria_postheader'><span class='Cancilleria_postheadericon'><a href='$node_url' title='$title'>$title</a></span></h2>";
  else
    $output = "<h1 class='Cancilleria_postheader'><span class='Cancilleria_postheadericon'>$title</span></h1>";
  return $output;
}
...
function art_hmenu_output($content) {
  return art_add_menu_class($content, 'Cancilleria_hmenu', true);
}
...
function art_vmenu_output($subject, $content) {
  ...
  $result = '';
  $vmenu = art_add_menu_class($content, 'Cancilleria_vmenu', true);
  $result .= <<< EOT
